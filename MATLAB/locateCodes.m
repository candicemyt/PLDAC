...
for i=1:numel(R)
    ...
    %x = [5.5/6 4.5/6 3.5/6 2.5/6 ];
    %xp = [repmat(x(1), 4, 1) ;repmat(x(2), 4, 1);repmat(x(3), 4, 1);repmat(x(4), 4, 1);repmat(x(5), 4, 1)];
    xp = [repmat(x(1), 5, 1) ;repmat(x(2), 5, 1);repmat(x(3), 5, 1);repmat(x(4), 5, 1);repmat(x(4), 5, 1)];
    %P = [xp  repmat(x,1,4)'];
    P = [xp  repmat(x,1,5)];
    f = [ 0 0;  0 1;  1  1;  1 0];
